...
/*===============================================================================================================================*/
unsigned int criar_vertice_buffer ( void *vertices, Vertex_array_config *array_config, int qtd_config, int qtd_data, int uso )
{
	unsigned int novaVAO;
	unsigned int novaVBO;

	unsigned int tam_data = 0;
	unsigned int offset = 0;

	for ( int i = 0; i < qtd_config; i++ ) {
		tam_data += tam_tipo_config( array_config[i].tipo ) * array_config[i].tam;
	}
	...
	for ( int i = 0; i < qtd_config; i++ ) {
		switch ( array_config[i].tipo_attrib ) {
			case Attrib_config::FLOAT:
				glVertexAttribPointer( i, array_config[i].tam, array_config[i].tipo, array_config[i].norm, tam_data, (void*)offset );
			break;
			case Attrib_config::INT:
				glVertexAttribIPointer( i, array_config[i].tam, array_config[i].tipo, tam_data, (void*)offset );
			break;
			case Attrib_config::DOUBLE:
				glVertexAttribLPointer( i, array_config[i].tam, array_config[i].tipo, tam_data, (void*)offset );
			break;
		}
		glEnableVertexAttribArray( i );
		
		offset += tam_tipo_config( array_config[i].tipo ) * array_config[i].tam;
	}
